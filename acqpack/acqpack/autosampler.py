...
class Autosampler:
    ...
    def add_frame(self, name, trans=np.eye(4,4), position_table=None):
        ...
        if isinstance(trans, str):
            trans = ut.read_delim_pd(trans).select_dtypes(['number']).to_numpy()
    ...
    def where(self, frame=None):
        ...
        """
        print(self.XY.where_xy()) # NEW
        print(self.Z.where()) # NEW
    ...
    def goto(self, frame, lookup_columns, lookup_values, zh_travel=0):
        ...
        if lookup_columns=='xyz':
            ...
            xh = lookup_values[0] # NEW
            yh = lookup_values[1] # NEW
            zh = lookup_values[2] # NEW
